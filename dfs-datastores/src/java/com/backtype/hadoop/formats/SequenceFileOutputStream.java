...
public class SequenceFileOutputStream implements RecordOutputStream {
    ...
    public SequenceFileOutputStream(FileSystem fs, Path path) throws IOException {
        _writer = SequenceFile.createWriter(fs.getConf(), SequenceFile.Writer.file(path), SequenceFile.Writer.keyClass(BytesWritable.class), SequenceFile.Writer.valueClass(NullWritable.class), SequenceFile.Writer.compression(CompressionType.NONE));
    }

    public SequenceFileOutputStream(FileSystem fs, Path path, CompressionType type, CompressionCodec codec) throws IOException {
        _writer = SequenceFile.createWriter(fs.getConf(), SequenceFile.Writer.file(path), SequenceFile.Writer.keyClass(BytesWritable.class), SequenceFile.Writer.valueClass(NullWritable.class), SequenceFile.Writer.compression(type, codec));
    }
